...
def read_movie_data(filename):
    ...
    with open(filename, encoding="utf-8") as f:
        for line in f:
            ...
            movie['Actors'] = movie['Actors'].split(", ")  # YOUR CODE HERE: Change this line to split `movie['Actors']` into a
...
def create_database_tables():
    with sqlite.connect(DB_PATH) as conn:
        ...
        cur.execute("CREATE UNIQUE INDEX IF NOT EXISTS uniqueindex ON actor (first_name, last_name)")
...
def insert_data(movies, actors):
    with sqlite.connect(DB_PATH) as conn:
        ...
        for m in movies:
            ...
            movie_rows.append((parse_imdb_id(m['imdbID']), replace_na(m['Title']), parse_int(m['Year']), replace_na(m['Director']), parse_float(m['Metascore']), parse_float(m['imdbRating'])))
        # print(movie_rows)
        # INSERT movie metadata
        cur.executemany("INSERT INTO movie VALUES (?,?,?,?,?,?)", movie_rows)
        conn.commit()
        ...
        for a in actors:
            ...
            first_name = replace_na(a[1])

            last_name = replace_na(a[2])

            if ((first_name,last_name)) not in actor_rows:
                actor_rows.append(( first_name, last_name))
            else:
                pass
        ...
        cur.executemany("INSERT INTO actor VALUES (NULL,?,?)", actor_rows)
        conn.commit()
        ...
        cur.execute("SELECT id,first_name,Last_name FROM actor")
        ...
        for a in actors:
            ...
            actor_id = actor_ids.get((a[1],a[2]))
            movie_imdb_id = parse_imdb_id(a[0] )  # YOUR CODE HERE: Replace None with correct value
            movie_actor_pairs.append((movie_imdb_id, actor_id))

        # INSERT connections between movies and actors
        cur.executemany("INSERT INTO movie_cast VALUES (?,?)", movie_actor_pairs)
        conn.commit()
...
def run_queries():
    ...
    with sqlite.connect(DB_PATH) as conn:
        ...
        cur.execute("SELECT COUNT(Title) FROM movie")
        result_set = cur.fetchall()
        ...
        # Your result should have: 1 column, 1 row.
        cur.execute("SELECT COUNT(id) FROM actor")
        result_set = cur.fetchall()
        ...
        cur.execute("SELECT last_name,COUNT(last_name) AS freq FROM actor GROUP BY last_name Order By freq DESC LIMIT 3")
        # cur.execute("SELECT last_name,first_name AS last_occurence FROM actor GROUP BY last_name,first_name ORDER BY last_occurence DESC")
        ...
        cur.execute("SELECT title FROM movie  ORDER BY imdb_rating DESC LIMIT 10")
        result_set = cur.fetchall()
        ...
        cur.execute("SELECT director,AVG(metascore) as average_metascore FROM movie GROUP BY director ORDER BY average_metascore DESC LIMIT 10")
        result_set = cur.fetchall()
        ...
        cur.execute("SELECT first_name, last_name, AVG(metascore) as average_metascore FROM movie, actor, movie_cast WHERE actor.id = movie_cast.actor_id AND movie_cast.movie_imdb_id = movie.imdb_id AND metascore IS NOT NULL GROUP BY first_name, last_name ORDER BY average_metascore DESC LIMIT 10")
        result_set = cur.fetchall()
        ...
        cur.execute("SELECT title, ABS(metascore - imdb_rating*10) as rating_difference FROM movie GROUP BY title ORDER BY rating_difference DESC Limit 5")
        result_set = cur.fetchall()
        ...
        # Your result should have: 2 columns, 5 rows
        cur.execute("SELECT year, AVG(metascore) as average_metascore FROM movie GROUP BY year ORDER BY average_metascore DESC LIMIT 5")
        result_set = cur.fetchall()
        ...
        ## Your result should have: 3 columns, 10 rows
        cur.execute("SELECT first_name, last_name, max(year)-min(year) as timespan FROM movie, actor, movie_cast WHERE actor.id = movie_cast.actor_id AND movie_cast.movie_imdb_id = movie.imdb_id AND metascore IS NOT NULL GROUP BY first_name, last_name ORDER BY timespan DESC LIMIT 10")
        result_set = cur.fetchall()
    ...
    with open("si-330-hw8-{}.json".format(YOUR_UNIQNAME), 'w') as f:
